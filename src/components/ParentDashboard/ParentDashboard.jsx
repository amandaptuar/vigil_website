...
export default function ParentDashboard() {
  ...
  const loadChildren = async () => {
    ...
    if (res.ok) {
      ...
      setChildren(list);
      if (list.length > 0 && !localStorage.getItem('vigil_childId')) {
  ...
  const handleLogout = async () => {
    ...
    ['vigil_token','vigil_parentId','vigil_childId','vigil_refreshToken','vigil_user'].forEach(k => localStorage.removeItem(k));
    navigate('/login');
  };
